...
CoCo supports many Trusted Execution Environments (TEE) including Intel TDX, AMD SEV, AMD SNP, and many more.This guide covers platform-specific setup for SEV(-ES) and walks through complete flows for attestation, encrypted images, and no pre-attestation.

## Creating a workload using NO pre-attestation on SNP
...
Ensure that you've followed the steps to prepare the host as stated [here](https://github.com/AMDESE/AMDSEV/tree/snp-latest). These instructions will help you setup SEV on your host level, which is required in order to create the SNP CoCo.

 For SEV with no pre-attestation, all that is required is the operator and the runtime class. The corresponding runtime class for launching with SEV is `kata-qemu-sev`. Now that you've installed and deployed the operator, you can go on to creating a kubernetes service yaml file. The yaml file is a config file that determines and tells how your kubernetes pods should run and interact with other objects. 
